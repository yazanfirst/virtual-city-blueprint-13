...
const InteriorScene = ({
  ...
  return (
    <group>
      {/* Lighting */}
      <ambientLight intensity={1.95} color="#fff8ec" />
      <hemisphereLight args={["#ffffff", "#1c1f2a", 1.1]} position={[0, 5, 0]} />
      <pointLight position={[0, 4.2, 0]} intensity={2.8} color={primary} decay={1.3} />
      <pointLight position={[2.5, 3.6, 2.5]} intensity={2.15} color={accent} decay={1.3} />
      <pointLight position={[-2.5, 3.6, 2.5]} intensity={2.15} color={accent} decay={1.3} />
      <spotLight
        position={[0, 4.5, -1]}
        intensity={2.5}
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#2b3145" roughness={0.62} metalness={0.1} />
      </mesh>

      {/* Back Wall */}
      <mesh position={[0, 2.5, -6]} receiveShadow>
        <boxGeometry args={[12, 5, 0.2]} />
        <meshStandardMaterial map={brickTexture || undefined} color="#bd7358" emissive="#2f130b" emissiveIntensity={0.16} />
      </mesh>
      ...
      <mesh position={[0, 5, 0]} rotation={[Math.PI / 2, 0, 0]} receiveShadow>
        <planeGeometry args={[12, 12]} />
        <meshStandardMaterial color="#2d3448" roughness={0.4} metalness={0.12} />
      </mesh>
    ...
    <div className="fixed inset-0 z-[250] bg-background/90 backdrop-blur-md flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-6xl h-[75vh] sm:h-[80vh] max-h-[calc(100vh-2rem)] cyber-card overflow-hidden border border-primary/30">
        ...
        <Canvas shadows camera={{ position: [0, 2.6, 2.2], fov: 55 }}>
          <color attach="background" args={["#16213b"]} />
          <fog attach="fog" args={["#16213b", 14, 26]} />
